...
class db{
    ...
    public function __construct($config=array()){
        ...
        $this->config['DB_USER'] = isset($config['DB_USER'])?$config['DB_USER']:'blue88888';//数据库用户名  蜜桃源码网
        $this->config['DB_PWD'] = isset($config['DB_PWD'])?$config['DB_PWD']:'6eHrHw58cfzKcTmw';//数据库密码
...
class mysql {
    ...
    public function connect($dbhost,$dbuser,$dbpw,$dbname='',$dbcharset='',$pconnect=false,$tablepre='') {
        ...
        $this->link = @mysqli_connect($host, $dbuser, $dbpw, $dbname, $port);
        if(!$this->link){
