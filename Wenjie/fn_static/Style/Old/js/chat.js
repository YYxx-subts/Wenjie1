...
function send_msg(msg){
	...
	if (msgtxt == "") {
		...
		if (!window.__pendingBetReq || window.__pendingBetReq.content !== msgtxt) {
			window.__pendingBetReq = { content: msgtxt, bet_req_id: (window.crypto && crypto.randomUUID ? crypto.randomUUID().replace(/\W/g, '') : (String(Date.now()) + Math.random().toString(16).slice(2))) };
		}
		$.ajax({
			url: '/Application/ajax_chat.php?type=send',
			type: 'post',
			data: { content: msgtxt, bet_req_id: window.__pendingBetReq.bet_req_id },
			dataType: 'json',
			success: function (data) {
				if (data.success) {
					window.__pendingBetReq = null;
			...
			error: function () { zy.tips('发送失败，请重试'); }
		});
	}
}
