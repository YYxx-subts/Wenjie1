...
(function (window) {
  ...
  function headerHtml(row, side) {
    var src = row && (row.headimg || row.avatar) ? String(row.headimg || row.avatar) : '';
    ...
    if (!src) return '';
    // 头像URL不需要HTML编码，直接使用原URL
    var img = '<img src="' + src + '" onerror="this.onerror=null;this.src=\'/Style/newimg/laba.png\'">';
