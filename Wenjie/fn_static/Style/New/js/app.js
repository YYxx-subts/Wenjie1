	...
	$('#sendbtn').click(function() { // 重点是这里，从这里向服务器端发送数据
		...
		if(msgtxt == ""){
			...
			$.ajax({
				...
				type: 'post',
				data: (function(){ if(!window.__pendingBetReq||window.__pendingBetReq.content!==msgtxt){ window.__pendingBetReq={content:msgtxt,bet_req_id:(window.crypto&&crypto.randomUUID?crypto.randomUUID().replace(/\W/g,''):(String(Date.now())+Math.random().toString(16).slice(2)))}; } return {content:msgtxt, bet_req_id:window.__pendingBetReq.bet_req_id}; })(),
				dataType: 'json',
