...
$view = isset($_GET['view']) ? (string)$_GET['view'] : 'game';
$pageTitle = ($view === 'bet') ? '竞猜列表' : '游戏记录';
...
$data = getorder($_SESSION['userid'], $tab);
$rows = array();
...
$self = 'orderinfo.php?view=' . rawurlencode($view);
?>
<!doctype html>
<html lang="zh-CN">
<head>
...
<div class="rl-page">
  ...
  <div class="rl-table-wrap">
    ...
    <div class="rl-list">
      ...
      <?php else: foreach ($rows as $con):
          $yk = isset($con[7]) ? $con[7] : '';
          $ykNum = is_numeric($yk) ? (float)$yk : 0;
          $ykClass = $ykNum > 0 ? 'rl-win' : ($ykNum < 0 ? 'rl-lose' : '');
      ?>
      <div class="rl-row">
        <span><?php echo htmlspecialchars((string)$con[2], ENT_QUOTES, 'UTF-8'); ?></span>
        <span><?php echo htmlspecialchars((string)$con[3], ENT_QUOTES, 'UTF-8'); ?></span>
        <span><?php echo htmlspecialchars((string)($con[4] . '/' . $con[5]), ENT_QUOTES, 'UTF-8'); ?></span>
        <span class="<?php echo $ykClass; ?>"><?php echo htmlspecialchars((string)$yk, ENT_QUOTES, 'UTF-8'); ?></span>
      </div>
